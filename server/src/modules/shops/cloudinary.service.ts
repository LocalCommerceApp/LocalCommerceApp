...
export class CloudinaryService {
  constructor(private configService: ConfigService) {
    const cloudName = (this.configService.get<string>('CLOUDINARY_CLOUD_NAME') || process.env.CLOUDINARY_CLOUD_NAME)?.trim();
    const apiKey = (this.configService.get<string>('CLOUDINARY_API_KEY') || process.env.CLOUDINARY_API_KEY)?.trim();
    const apiSecret = (this.configService.get<string>('CLOUDINARY_API_SECRET') || process.env.CLOUDINARY_API_SECRET)?.trim();
